...
const consoleindev=require("../utils/console")
...
const {sliceOccurences,generateRandomString,sliceOccurencesfornonvolume}=require("../utils/datautils")
const fintnessModule = {
    ...
    showWorkoutreport:async (req, res) => {
        ...
        const workoutreports1=await db.workout_schedule.sync().then(()=>{
        ...
        const workoutreport2=await db.workout_schedule_without_volume.sync().then(()=>{
            return db.workout_schedule_without_volume.findAll({
                where:{
                    identifier:identifier
                }
            })
        })
        const workoutreports=workoutreports1.length>0?workoutreports1:workoutreport2
...
renderGenerateWorkoutReportViewv3: async (req, res) => {
    ...
    renderGenerateWorkoutReportViewv4: async (req, res) => {
        ...
        fetchgoalsbyactivityid:async (req, res) => {
            ...
            consoleindev(typeof activity_id)
            let goals;
            if(activity_id=='4'){
                goals=await db.goalswithoutvolume.findAll({
                    where:{
                        activity_id:activity_id
                    },
                    attributes: [[db.Sequelize.fn('DISTINCT', db.Sequelize.col('goal')), 'goal']]
        ...
        handleweekcalulation:async(startdate,combosArray,randomStr,goal,activity_id)=>{
            let currentweek = 1;
            let dataforweek= 1;
            let currentdate = undefined;
            ...
            let newstartday = getNextCycleDate(startdate, combosArray);
            // console.log("randomStr", randomStr)
            let model=activity_id=='4'?db.workout_schedule_without_volume:db.workout_schedule
            if (randomStr != undefined && randomStr != null && randomStr != "") {
                prevWeek = await model.max('week', {
                    where: {
                        identifier: randomStr
                    }
                });
                consoleindev("randomStr",randomStr)
                consoleindev("prevWeek",prevWeek)
                prevDate = await model.max('date', {
                    where: {
                        identifier: randomStr,
                        week: prevWeek
                    }
                })
                consoleindev("prevDate",prevDate)
                let lastweekgoal = await model.findOne({
                    where: {
                        identifier: randomStr,
                        week: prevWeek,
                        date:prevDate
        ...
        generatereportv5: async (req, res) => {
            const { startdate, daysoptions, comboselem, phaseno, goal, uid, useridentifier, activity_id } = req.body;
            const { changeintotalpercentageforage, changeintotalpercentageforfitnesslevel } = await fintnessModule.rulescalculations(useridentifier, activity_id)
            ...
            const totalweeks = 1;
            const totaldays = totalweeks * 7;
            let { currentweek, dataforweek, newstartday } = await fintnessModule.handleweekcalulation(startdate, combosArray, randomStr, goal,activity_id)
        
            // console.log("dataforweek",dataforweek)
           
            const paramsforgenertevolumedata = {
                activity_id,
                dataforweek,
                newstartday,
                ...
                changeintotalpercentageforage,
                changeintotalpercentageforfitnesslevel,
                daysoptions,
                comboselem
            }
           
            const alldates = activity_id=='4'?await fintnessModule.generatenonvolumedata(paramsforgenertevolumedata):await fintnessModule.generatevolumedata(paramsforgenertevolumedata)
            let randomstring = randomStr ? randomStr : generateRandomString(12)
        ...
        savevolumedatatodb: async ({randomstring,alldates,comboselem,unitofexersice,goal,daysoptions}) => {
        ...
        savenonvolumedatatodb: async ({randomstring,alldates,comboselem,unitofexersice,goal,daysoptions}) => {
            ...
            for (let k = 0; k < alldates.length; k++) {
                ...
                for (let l = 0; l < newweek.length; l++) {
                    let newday = newweek[l]
                    const day=await db.workout_schedule_without_volume.create({
                        numofrunningdays: parseInt(daysoptions),
                        daysoption: parseInt(comboselem),
                        week: alldates[k].week,
                        identifier: randomstring,
                ...
                datatosend.push({week:alldates[k].week,weekdates:weektopush})
    
            }
            return datatosend
    },
        generatevolumedata:async(
            {
            activity_id,
            dataforweek,
            newstartday,
            ...
            for (let i = 0; i < totaldays; i++) {
                ...
                consoleindev("newstartday",newstartday)
                let date = new Date(newstartday);
                date.setDate(date.getDate() + i);
                let newdate = date.toISOString().split("T")[0];
                let newday = date.getDay();
        
                consoleindev("newdate",newdate)
                ...
                if ((i + 1) % 7 === 0) {
                    ...
                    let totalquota = weekdata[daysoptionsmapping[daysoptions]]
                //    consoleindev("totalquota",totalquota)
                //    consoleindev(daysoptionsmapping[daysoptions],daysoptionsmapping[daysoptions])
                //    consoleindev("devoptions",daysoptions)
                    //1 st rule base on age
                    totalquota = totalquota + (totalquota * (changeintotalpercentageforage / 100)) + ((totalquota * (changeintotalpercentageforfitnesslevel / 100)))
        ...
        generatenonvolumedata:async(
            ...
            let goaldata = await db.goalswithoutvolume.findAll({ where: { goal: goal,activity_id:activity_id+'' },
            include: [
                { model: db.subworkout, as: '1' },
                { model: db.subworkout, as: '2' },
                { model: db.subworkout, as: '3' },
                { model: db.subworkout, as: '4' },
                { model: db.subworkout, as: '5' },
                { model: db.subworkout, as: '6' },
                { model: db.subworkout, as: '7' },
            ]
        
                })
        
            let weeknumber = dataforweek
            sliceOccurencesfornonvolume(goaldata,weeknumber, 0)
            // return goaldata
            // consoleindev("goaldata",goaldata)
            // console.log("before numberofoccurenceMutable",numberofoccurenceMutable)
            for (let i = 0; i < totaldays; i++) {
                ...
                //    consoleindev("devoptions",daysoptions)
                    ...
                    for (index = 0; index < week.length; index++) {
                        const activityfoday=goaldata[0]
                        const day = week[index]
                        ...
                        day.sub_workout_id = activityfoday[index+1].id
                        // // console.log("day.newday",day.newday)
                        day.workoutname =workoutnamemapping.find(e=>e.day==day.newday).value
                        day.subworkout = activityfoday[index+1].subworkout
                    ...
                    alldates.push({ week: currentweek, weekdates: [...week] });
                    week = [];
                }
